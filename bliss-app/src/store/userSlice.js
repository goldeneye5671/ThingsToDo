...
export const updateUserDescription = createAsyncThunk("user/updateUserDescription", async (description, {getState, dispatch}) => {
    const accessToken = getState()?.session?.user?.accessToken;
    const response = await axios.patch(
...
export const updateUserExperience = createAsyncThunk("user/updateUserExperience", async (experience, {getState, dispatch}) => {
    const accessToken = getState()?.session?.user?.accessToken;
    const response = await axios.patch(
...
export const userSlice = createSlice({
    ...
    extraReducers(builder) {
        ...
        .addCase(updateUserDescription.fulfilled, (state, action) => {
            state.status="fulfilled";
            const index = state.user.CustomDescriptions.findIndex((desc) => desc.id === parseInt(action.payload.id));
        ...
        .addCase(deleteUserDescription.fulfilled, (state, action) => {
            state.status="fulfilled";
            state.user.CustomDescriptions = state.user.CustomDescriptions.filter((desc) => desc.id !== parseInt(action.payload.id));
        })
        ...
        .addCase(updateUserExperience.fulfilled, (state, action) => {
            state.status="fulfilled";
            const index = state.user.Experiences.findIndex((exp) => exp.id === parseInt(action.payload.id));
        ...
        .addCase(deleteUserExperience.fulfilled, (state, action) => {
            state.status="fulfilled";
            state.user.Experiences = state.user.Experiences.filter((exp) => exp.id !== parseInt(action.payload.id));
        })
